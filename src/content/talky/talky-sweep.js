// TESSERACT - Módulo SWEEPS (LFP context + Saludos + Cartas + Star Tools)
...
async function executeSaludos() {
  ...
  const list = document.querySelector(TALK_Y.CONTACT_LIST_PRIMARY);
  if (!list) {
    console.log('[SALUDOS] ❌ Lista no encontrada con selectores. Buscando lista genérica...');
    const altList = document.querySelector(TALK_Y.CONTACT_LIST_ALT);
    if (!altList) {
