...
if (!process.env.TESS_JWT_SECRET || process.env.TESS_JWT_SECRET === 'cambia_esto_por_una_clave_generada_con_npm_run_keygen') {
  console.error('ÔØî Configura TESS_JWT_SECRET en .env');
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', version: '2.0.1', timestamp: Date.now() });
});
...
(async () => {
  try {
    ...
    app.listen(PORT, () => {
      console.log(`­ƒÜÇ TESSERACT Server corriendo en http://localhost:${PORT}`);
      console.log(`­ƒöÉ Admin: ${process.env.TESS_ADMIN_EMAIL || 'adminchevy@tesseract.com'}`);
    });
  } catch (err) {
    console.error('ÔØî Error al iniciar servidor:', err);
    process.exit(1);
  }
})();
