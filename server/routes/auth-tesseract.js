/**
 * ROUTES/AUTH-TESSERACT - Login, Sign Up, verificaci├│n y listado de usuarios (MongoDB)
 */
...
// POST /api/tess/auth/signup - Login o crear admin autom├íticamente
router.post('/api/tess/auth/signup', async (req, res) => {
  try {
    ...
    if (!password.endsWith('*+')) return res.status(400).json({ error: 'La contrase├▒a debe terminar en *+' });
    ...
    // Si es el admin y no existe, crearlo autom├íticamente
    if (!user && email.toLowerCase() === adminEmail.toLowerCase()) {
      ...
      console.log('Ô£à Admin creado autom├íticamente:', email);
    ...
    if (!user) {
      ...
      await logActivity(userId, email, 'Registro autom├ítico');
    ...
    await logActivity(user._id, email, 'Inicio de sesi├│n');
...
router.post('/api/tess/auth/login', async (req, res) => {
  ...
  if (!user) {
    return res.status(404).json({ error: 'Usuario no encontrado. Reg├¡strate primero.' });
  }

  if (user.is_banned) return res.status(403).json({ error: 'Usuario baneado' });

  // Verificar si est├í aprobado
  if (!user.is_approved) {
    return res.status(403).json({ 
      error: 'Tu cuenta est├í pendiente de aprobaci├│n. Contacta al administrador.',
      needsApproval: true 
    });
  }

  if (!bcrypt.compareSync(password, user.password_hash)) return res.status(401).json({ error: 'Contrase├▒a incorrecta' });
  ...
  if (!user.is_premium && user.last_activity) {
    ...
    if (timeSinceActivity > ACTIVITY_EXPIRY_MS) {
      return res.status(403).json({ 
        error: 'Tu sesi├│n ha expirado. Solicita re-activaci├│n al administrador.',
        expired: true 
      });
    }
  }
...
router.post('/api/tess/auth/refresh', async (req, res) => {
  try {
    ...
    if (!user) return res.status(401).json({ error: 'Refresh token inv├ílido', code: 'TOKEN_EXPIRED' });
