/**
 * ROUTES/ADMIN-TESSERACT - Premium, ban, developer, actividad y m├®tricas globales
 */
...
// Obtener usuarios con m├®tricas del d├¡a
...
// Obtener m├®tricas diarias por oficina (calendario)
router.get('/api/tess/admin/metrics-daily', requireTesseractAdmin, enforceOfficeFilter, async (req, res) => {
  try {
    ...
    // Agregar pipeline para obtener m├®tricas diarias por fecha
...
// Obtener m├®tricas por usuario espec├¡fico
router.get('/api/tess/admin/metrics-by-user', requireTesseractAdmin, enforceOfficeFilter, async (req, res) => {
  ...
  // Office admin: verificar que el usuario est├® en su oficina
  if (req.user.is_office_admin && !req.user.is_developer && !req.user.is_admin) {
    if (user.office !== req.user.office) {
      return res.status(403).json({ error: 'No tienes permiso para ver m├®tricas de usuarios de otra oficina' });
    }
  }
...
router.post('/api/tess/admin/developer', requireMasterAdmin, async (req, res) => {
  ...
  if (!email || !action) return res.status(400).json({ error: 'Email y acci├│n requeridos' });
  if (!['add', 'remove'].includes(action)) return res.status(400).json({ error: 'Acci├│n: add o remove' });
  const user = await findUserByEmail(email);
  if (action === 'add') {
    if (!user) {
...
router.post('/api/tess/admin/set-password', requireTesseractAdmin, requireOfficeScoped, async (req, res) => {
  ...
  await logActivity(req.user._id.toString(), req.user.email, `Contrase├▒a cambiada para: ${email}`);
  res.json({ success: true });
});
