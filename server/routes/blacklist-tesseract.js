/**
 * ROUTES/BLACKLIST-TESSERACT - Gesti├│n de lista negra de contactos
 */
...
router.post('/api/tess/blacklist/add', validateToken, async (req, res) => {
  try {
    ...
    await logActivity(userId, req.user.email, 'BLACKLIST', `a├▒adi├│ ${contactId}`);
...
router.post('/api/tess/blacklist/remove', validateToken, async (req, res) => {
  try {
    ...
    await logActivity(userId, req.user.email, 'BLACKLIST', `elimin├│ ${contactId}`);
...
// POST /api/tess/blacklist/check - Verificar si un contacto est├í en blacklist
...
// GET /api/tess/blacklist/check/:contactId - Verificar si est├í en blacklist (para Auto Answer)
