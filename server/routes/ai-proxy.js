...
router.post('/api/chatgpt/chat', validateToken, async (req, res) => {
  try {
    ...
    console.error('[AI-PROXY] Groq fall├│:', JSON.stringify({ status: groqResult.status, error: groqResult.data?.error || groqResult.data }));
    ...
    console.error('[AI-PROXY] OpenAI fall├│:', JSON.stringify({ status: openaiResult.status, error: openaiResult.data?.error || openaiResult.data }));
...
// POST /api/openai/translate - Traducci├│n
router.post('/api/openai/translate', validateToken, async (req, res) => {
  try {
    ...
    const systemMsg = `Traduce el siguiente texto del espa├▒ol al ${langName} (${langCode}). Responde SOLO con la traducci├│n, sin explicaciones ni notas.`;
...
router.post('/api/deepl/translate', validateToken, async (req, res) => {
  try {
    ...
    const systemMsg = `Traduce al ${target || 'espa├▒ol'}. Solo responde con el texto traducido.`;
