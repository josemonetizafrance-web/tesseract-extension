...
// NOTE: MONGODB_URI debe estar en .env - NO hardcodear credenciales aqu├¡
const MONGODB_URI = process.env.MONGODB_URI;
if (!MONGODB_URI) {
  console.error('ÔØî Error: Define MONGODB_URI en el archivo .env');
...
async function connectMongo() {
  try {
    ...
    console.log('Ô£à Conectado a MongoDB Atlas');
    return db;
  } catch (err) {
    console.error('ÔØî Error conectando a MongoDB:', err.message);
    throw err;
  }
}
...
async function initDb() {
  ...
  // Crear usuario admin si no existe (solo si est├ín definidos en .env)
  const adminEmail = process.env.TESS_ADMIN_EMAIL;
  const adminPass = process.env.TESS_ADMIN_PASSWORD;
  
  if (!adminEmail || !adminPass) {
    console.log('ÔÜá´©Å  Admin no creado: define TESS_ADMIN_EMAIL y TESS_ADMIN_PASSWORD en .env');
  } else {
  ...
  if (!existingAdmin) {
    ...
    console.log('Ô£à Admin creado:', adminEmail);
  } else {
    ...
    console.log('Ô£à Admin verificado:', adminEmail);
  }
  }
   
  // Heartbeat cada 4 minutos para mantener conexi├│n activa en free tier
  setInterval(async () => {
    try { await db.command({ ping: 1 }); } catch (e) {}
  }, 240000);

  console.log('Ô£à Base de datos MongoDB inicializada');
  return db;
}
...
async function createUserPending(email, passwordHash, demoExpiry) {
  ...
  const result = await db.collection('tess_users').insertOne({
    ...
    is_developer: 0,
    is_banned: 0,
    is_approved: 0, // Pendiente de aprobaci├│n
    is_premium: 0,
    demo_expiry: demoExpiry,
    premium_expiry: null,
    login_count: 0,
...
// Exponer db para operaciones directas (usar con precauci├│n)
function getDb() {
